...
export class ModelMetadata {
  ...
  static createListingMarkdown(
    ...
    if (ModelMetadata.verbose) {
      console.log(`Creating markdown for ${listing.file}...`);
    }
  ...
  static createModelIndexJson(models: Model[]): any {
    if (ModelMetadata.verbose) {
      console.log(`Creating model index for ${models.length} models...`);
    }
    const modelIndex = [];
    ...
    if (ModelMetadata.verbose) {
      console.log(`Creating model index for ${models.length} models DONE`);
    }
    return modelIndex;
  }
  ...
  private static createDep5(baseDirectory: string, models: Model[]): string {
    if (ModelMetadata.verbose) {
      console.log(`Creating dep5 for ${models.length} models...`);
    }
    ...
    if (ModelMetadata.verbose) {
      console.log(`Creating dep5 for ${models.length} models DONE`);
    }
    return result;
  }
  ...
  private static updateModelFiles(baseDirectory: string, model: Model) {
    if (ModelMetadata.verbose) {
      console.log("Update files for " + model.getName());
    }
